...
CREATE TABLE IF NOT EXISTS sbt_issuance (
  ...
  wallet_address VARCHAR(42) NOT NULL UNIQUE,  -- 지갑 주소 (0x + 40자) - 학회원당 1개만 발급 - personal_info 참조
  ...
  CONSTRAINT fk_sbt_issuance_wallet
...
-- 인덱스 생성 (성능 최적화)
-- 특정 사용자의 SBT 조회 최적화
CREATE INDEX IF NOT EXISTS idx_sbt_issuance_wallet
  ON sbt_issuance(wallet_address);
...
COMMENT ON COLUMN sbt_issuance.wallet_address IS '수료증 수령자 지갑 주소 (0x + 40자 Ethereum 주소) - 학회원당 1개만 발급 - personal_info 테이블 참조';
