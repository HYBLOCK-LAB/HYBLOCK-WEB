...
import { academicStatusLabels } from '@/lib/recruitment';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rows = (applications ?? []).map((application: any) => {
      ...
      const fixed = [application.name,application.birth_year,application.university,application.major,application.student_id,academicStatusLabels[application.academic_status] ?? application.academic_status,application.email,application.phone,application.recruitment_track?.label ?? '',applicationStatusLabels[application.status] ?? application.status,application.submitted_at,autoScore + manualScore,interviewScore];
