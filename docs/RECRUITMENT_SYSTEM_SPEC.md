...
- 지원자는 로그인, 지갑 연결, `member` 등록 없이 공개 `/apply` 페이지에서 지원서를 작성한다.
- 제출된 지원자 개인정보와 지원 내용은 관리자만 조회할 수 있다.
- 지원자용 결과 조회 페이지와 제출 후 지원서 재조회 기능은 제공하지 않는다.
- 합격·불합격 결과는 운영진이 지원서의 이메일 또는 전화번호를 이용해 웹사이트 밖에서 직접 안내한다.
...
- 동일인이 여러 번 제출했는지는 1차 범위에서 판별하거나 차단하지 않는다.
- 서버 임시 저장은 제공하지 않는다.
...
- 동일 제출 요청의 중복 클릭이나 네트워크 재시도로 두 건이 생성되지 않도록 idempotency key와 DB unique constraint를 적용한다.
- 동일 이메일·전화번호의 별도 제출은 차단하지 않는다.
...
| `open` | 지원 접수 중 | 가능 | 불가 |
| `closed` | 접수 마감 | 불가 | 불가 |
| `selection_complete` | 선발 완료 및 개별 연락 단계 | 불가 | 불가 |
...
- 지원 상태는 관리자 화면에서만 사용하며 지원자용 웹 화면에는 노출하지 않는다.
...
- 이메일은 지원자가 직접 입력하고 형식을 서버에서 검증한다.
...
- `idempotency_key` UUID unique: 동일 제출 요청 재처리 방지
- `created_at`
...
- 지원자용 읽기/제출 API는 공개 endpoint이지만 모든 DB 쓰기는 Route Handler의 서버 client를 통해서만 처리한다. 브라우저가 recruitment 테이블에 직접 insert하지 않는다.
- 지원자는 열린 모집 정보와 질문을 읽을 수 있지만 정답 점수(`auto_score`)와 면접 평가 문항은 볼 수 없다.
- 제출 후 지원서·상태·점수·관리자 코멘트를 읽는 공개 API는 만들지 않는다.
- 공개 제출 API에는 IP 기반 rate limit, 요청 크기 제한, honeypot 필드, 엄격한 서버 검증을 적용한다. 필요 시 CAPTCHA를 추가할 수 있도록 경계를 둔다.
...
- 제출 직후에만 제출 번호, 제출 시각, 모집명, 지원 파트를 표시한다.
- 새로고침이나 URL 직접 접근으로 지원서 원문을 다시 조회하지 않는다.
- 결과는 입력한 이메일 또는 전화번호로 운영진이 별도 안내한다는 문구를 표시한다.
- 조회 코드나 결과 확인 링크는 발급하지 않는다.
...
- 지원 상태별 필터: 전체, 심사 중, 서류 합격, 서류 불합격, 면접, 최종 합격, 최종 불합격
- 지원 파트와 학교 필터
- 이름 검색
- 출생연도, 제출일, 지원서 점수, 면접 점수를 각 열 헤더에서 오름차순·내림차순 정렬
- 점수 정렬 시 확정 점수를 우선 사용하고, 미확정 점수는 `잠정` 표식과 함께 정렬하며 완전 미평가 값은 항상 마지막에 배치
- 필터와 정렬은 서버 pagination보다 먼저 적용하고 URL query string에 보존하여 새로고침·링크 공유 시 같은 목록 상태를 유지
- 필터 조합을 허용한다. 예: `최종 합격 + 개발팀 + 지원서 점수 내림차순`
...
- 관리자 인증 실패는 `401`, 권한 부족은 `403`, 상태 충돌·동일 idempotency key 재사용은 `409`, 검증 실패는 `400` 또는 `422`, rate limit 초과는 `429`를 사용한다.
...
- 미평가 항목의 진행률 계산
- 마감 전/후 제출 가능 여부
- 동일 idempotency key를 사용한 중복 요청 방지
- 공개 제출 API의 rate limit과 honeypot 차단
- 제출 후 질문 및 답변 수정 거절
- evaluator가 아닌 관리자의 평가 쓰기 거절
- 다른 지원자의 지원서 조회 거절
- 지원자용 상태·점수·코멘트 조회 API가 존재하지 않음
- 상태 전환 및 이력 생성
...
- 로그인하지 않은 지원자가 열린 모집에 지원서를 제출할 수 있다.
...
- 지원 상태 변경 이력이 남고 지원 상태·점수·코멘트가 공개 API로 노출되지 않는다.
...
- 지원자용 결과 확인 페이지는 제공하지 않는다.
- 합격·불합격 결과는 시스템이 자동 발송하지 않으며 운영진이 저장된 이메일 또는 전화번호로 직접 안내한다.
