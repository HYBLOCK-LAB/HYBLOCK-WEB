...
CREATE TABLE IF NOT EXISTS semester_criteria_tracking (
  ...
  member_id INTEGER NOT NULL,  -- 멤버 ID - member 참조
  ...
  CONSTRAINT fk_semester_criteria_member
...
CREATE INDEX IF NOT EXISTS idx_semester_criteria_member
  ON semester_criteria_tracking(member_id);
...
CREATE INDEX IF NOT EXISTS idx_semester_criteria_member_type
  ON semester_criteria_tracking(member_id, criteria_type);

-- 복합 인덱스: 사용자별 기수 조회 최적화 (2학기 연속 조건 확인)
CREATE INDEX IF NOT EXISTS idx_semester_criteria_member_cohort
  ON semester_criteria_tracking(member_id, cohort);
...
COMMENT ON COLUMN semester_criteria_tracking.member_id IS '학회원 멤버 ID - member 테이블 참조';
COMMENT ON COLUMN semester_criteria_tracking.cohort IS '기수/학기 (attendance_session.cohort와 대응) - 1, 2, 3...';
