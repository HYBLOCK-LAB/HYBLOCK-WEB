...
CREATE TABLE IF NOT EXISTS assignment (
  ...
  CONSTRAINT fk_assignment_wallet
  ...
  CONSTRAINT uq_assignment_evidence
    UNIQUE (wallet_address, session_id, evidence_url)
);

-- 인덱스 생성 (성능 최적화)
CREATE INDEX IF NOT EXISTS idx_assignment_wallet
  ON assignment(wallet_address);
...
CREATE INDEX IF NOT EXISTS idx_assignment_wallet_session
  ON assignment(wallet_address, session_id);
...
COMMENT ON COLUMN assignment.wallet_address IS '참여자 지갑 주소 (0x + 40자 Ethereum 주소) - personal_info 테이블 참조';
COMMENT ON COLUMN assignment.session_id IS '학기 세션 ID - session 테이블 참조';
