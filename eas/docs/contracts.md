...
#### `issue(address walletAddress, bytes32 personalDataHash, string attestationType, string revealedData, bool isGraduated) → bytes32`
- 권한: `admin`
...
**스키마 인코딩 순서**
```
abi.encode(address, bytes32, string, string, bool)
           wallet   hash    type    revealed  isGrad
```
